...
from models.ranker import pad_fn
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    log.console(args)
    if args.wandb_on:
        wandb.init(project="SimCKP-S2", name="/".join(args.train_output_dir.split("/")[1:]))
...
class Trainer:
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            # avg_valid_loss, _ = self.__epoch_valid()
                ...
                if step % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.global_step == 1 or self.global_step % self.args.logging_steps == 0:
                        log.console(f"current lr: {self.optimizer.param_groups[0]['lr']:.8f}, " +
                                    f"steps: {self.global_step}, " +
                                    f"train loss: {(train_loss / (step / self.args.gradient_accumulation_steps)):.4f}")
                        if self.args.wandb_on:
                            wandb.log({"Train Loss": train_loss / (step / self.args.gradient_accumulation_steps)}, step=self.global_step)
    ...
    def __epoch_valid(self):
        ...
        with tqdm(desc="Validating", total=total, ncols=100, disable=self.args.hide_tqdm) as pbar:
            for step, inputs in enumerate(self.valid_loader, 1):
                ...
                ### Forward pass ###
                loss, logits, labels = self.model(**inputs)
        ...
        if self.args.wandb_on:
            wandb.log({"Valid Loss": valid_loss / total,
                       "P@5": score_dict['P@5'], "R@5": score_dict['R@5'], "F1@5": score_dict['F1@5'],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KP Stage 2 Training")
